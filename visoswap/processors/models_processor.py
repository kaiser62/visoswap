...
from visoswap.processors.utils.engine_builder import onnx_to_trt as onnx2trt
from visoswap.processors.utils.tensorrt_predictor import TensorRTPredictor
from visoswap.processors.face_detectors import FaceDetectors
from visoswap.processors.face_landmark_detectors import FaceLandmarkDetectors
from visoswap.processors.face_masks import FaceMasks
from visoswap.processors.face_restorers import FaceRestorers
from visoswap.processors.face_swappers import FaceSwappers
from visoswap.processors.frame_enhancers import FrameEnhancers
from visoswap.processors.face_editors import FaceEditors
from visoswap.processors.utils.dfm_model import DFMModel
from visoswap.models.models_data import models_list, arcface_mapping_model_dict, models_trt_list
from visoswap.processors.utils.misc import is_file_exists
...
class ModelsProcessor:
    ...
    def __init__(self, context: 'EngineContext', device='cuda'):
        super().__init__()
    ...
    def load_dfm_model(self, dfm_model):
        with self.model_lock:
            if not self.dfm_models.get(dfm_model):
                max_models_to_keep = self.context.control['MaxDFMModelsSlider']
                ...
                if total_loaded_models==max_models_to_keep:
                    ...
                    gc.collect()
                try:
                    self.dfm_models[dfm_model] = DFMModel(self.context.dfm_models_data[dfm_model], self.providers, self.device)
                except:
                    traceback.print_exc()   
