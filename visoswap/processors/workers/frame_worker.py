...
from visoswap.processors.utils import faceutil
from visoswap.processors.utils.misc import t512,t384,t256,t128
...
class FrameWorker(threading.Thread):
    def __init__(self, frame, context: 'EngineContext', frame_number, is_single_frame=False):
        super().__init__()
        self.frame = frame
        self.context = context
        self.frame_number = frame_number
        self.models_processor = context.models_processor
        self.is_single_frame = is_single_frame
        self.parameters = {}
        self.target_faces = context.target_faces
        self.compare_images = []

    def run(self):
        try:
            # VisoSwap: upstream updated parameters from timeline markers here, under
            # models_processor.model_lock. Both the call and the lock -- which guarded
            # nothing else -- are removed; timeline markers are a host concern.
            ...
            if self.context.swap_faces_enabled or self.context.edit_faces_enabled or self.context.control['FrameEnhancerEnableToggle']:
            ...
            # VisoSwap: upstream converted the frame for display here and handed it to
            # the playback loop -- three emissions branching on webcam / video /
            # single frame, the queue drain, and the playback-completion stop. All of
            # that is the host application's display path and is removed.
            #
            # The processed frame is left on self.frame and has no route out. That is
            # the intended seam: Phase 2's Engine.swap() defines how a caller
            # retrieves it. Do not invent a queue, a callback or a return protocol
            # here -- Phase 2 owns that decision.
    ...
    def is_compare_mode(self):
        for _, target_face in self.context.target_faces.items():
    ...
    def process_frame(self):
        ...
        if self.context.edit_faces_enabled:
        ...
        if det_faces_data:
            ...
            for i, fface in enumerate(det_faces_data):
                    for _, target_face in self.context.target_faces.items():
                        ...
                        if self.context.swap_faces_enabled or self.context.edit_faces_enabled:
                            ...
                            if sim>=parameters['SimilarityThresholdSlider']:
                                ...
                                if self.context.swap_faces_enabled:
